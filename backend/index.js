...
mongoose.connect('mongodb+srv://jb:anniebannanniefeefifophannie@menu-data-cluster.cjjaxoh.mongodb.net/?retryWrites=true&w=majority&appName=menu-data-cluster');
...
*
*/
app.get("/fill-data/81mnOARTS7GfgTCdfG061tyekAjZhuTNAC3qAEObKK8s3Bn4FEhtw10ardgAX6FjVT57IZULTVAoVnNs2v9XkJ2rgMs7gKR3y7IFuqWBTZpCuflZBF3yMMh6Yl3cr98cGDJCxc3jNeiPPwQuIprnQqVWnLbdxiOl", async (req, res) => {
    try {
...
app.post("/menu", (req, res) => {
    ...
    MenuItemModel.create({location: location, item: item, date: date, station: station, time: time, dateAdded: dateAdded})
...
app.get("/menu/search", async (req, res) => {
    try {
        ...
        const filteredItems = await MenuItemModel.find({ date: { $gte: currentDate } });
...
app.delete("/del-all-data", async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `${deleteResult} data points deleted successfully.` });
    } catch (error) {
        console.error('Error deleting data:', error);
        res.status(500).json({ error: 'An error occurred while deleting data points.' });
    }
});
